  ...
  */
  ...
  let restaurantFinderApplication = {
    ...
    findAvailableRestaurants: function (numberOfPeople) {
      return restaurants.map(function(min){
          if(min.totalSeats - min.numberOfCustomers > numberOfPeople){
            return min.name;
          }
        }
      )
    ...
    findRestaurantServingDish: function (dishName) {
      return restaurants.map((disi) => {
        if (disi.menu.includes(dishName)){
          return disi.name;
        }
      })
    },
    countNumberOfRestaurantsInArea: function (area) {
      return restaurants.filter((uni) => uni.address.area == area).length;
    },
  };
